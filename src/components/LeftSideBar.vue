    ...
    <div class="select-option-class">
      ...
      <div class="icon">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
          <g fill="currentColor">
            <path
              d="M7.75 2.5a.75.75 0 0 0-1.5 0v1.58c-1.44.115-2.384.397-3.078 1.092c-.695.694-.977 1.639-1.093 3.078h19.842c-.116-1.44-.398-2.384-1.093-3.078c-.694-.695-1.639-.977-3.078-1.093V2.5a.75.75 0 0 0-1.5 0v1.513C15.585 4 14.839 4 14 4h-4c-.839 0-1.585 0-2.25.013V2.5Z" />
      ...
      <div class="icon">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 48 48">
          <mask id="ipSDegreeHat0">
            <g fill="none" stroke="#fff" stroke-linejoin="round" stroke-width="4">
              <path fill="#fff" d="M2 17.4L23.022 9l21.022 8.4l-21.022 8.4L2 17.4Z" />
              <path stroke-linecap="round"
                d="M44.044 17.51v9.223m-32.488-4.908v12.442S16.366 39 23.022 39c6.657 0 11.467-4.733 11.467-4.733V21.825" />
            </g>
          </mask>
          <path fill="currentColor" d="M0 0h48v48H0z" mask="url(#ipSDegreeHat0)" />
        </svg>
      </div>
      ...
      <div class="icon">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
          <path fill="currentColor"
            d="M5 22h14c1.103 0 2-.897 2-2V6c0-1.103-.897-2-2-2h-2V2h-2v2H9V2H7v2H5c-1.103 0-2 .897-2 2v14c0 1.103.897 2 2 2zm4.799-2.013H8v-1.799l4.977-4.97l1.799 1.799l-4.977 4.97zm5.824-5.817l-1.799-1.799L15.196 11l1.799 1.799l-1.372 1.371zM5 7h14v2H5V7z" />
        </svg>
      </div>
      <select v-model="selectedSemester" class="form-select" @change="emitSemesterSelected">
        <option v-for="semester in fetchedSemesters" :key="semester.id" :value="semester.id">{{ semester.name_en }}
        </option>
      </select>
    </div>
...
export default defineComponent({
  ...
  methods: {
    setRequireField(localData){
      if (localData) {
        this.fetchedAcademyYears = localData?.academic_years;
        this.selectedAcademyYear = this.fetchedAcademyYears[0]?.id;
        this.fetchedDepartments = localData?.departments;
        this.selectedDepartment = this.fetchedDepartments[0]?.id;
    ...
    // Other fetch methods...
    async fetchData() {
      const apiUrl = 'http://127.0.0.1:8000/api/fields';
      let response = await axios.get(apiUrl);
      let localData = response.data;
      this.setRequireField(localData);
  ...
  async created() {
    await this.fetchData().then(value => {
